...
if has('vim_starting')
  set viminfo& viminfo+='1000,<50,s10,h
endif
...
" 定義ファイルの配置場所指定
let g:neosnippet#snippets_directory = $XDG_CONFIG_HOME . '/nvim/snippet'
...
call lexima#add_rule({ 'filetype': ['ruby', 'eruby', 'haml'], 'char': ':', 'input': ':'})
...
let g:memolist_path = $XDG_CONFIG_HOME . '/memo'
...
command! ERunVimrc edit $XDG_CONFIG_HOME/nvim/run.vim
command! EFiletypeSetting edit $XDG_CONFIG_HOME/nvim/autoload/my/filetype.vim
command! ReflectVimrc source $MYVIMRC
command! RunVimScript source $XDG_CONFIG_HOME/nvim/run.vim
...
" _を少し拡張(カウントは使わない前提)
" https://github.com/yuki-yano/dotfiles/blob/7f10462d12228f57ea3cf86d61f3b22d4728b6f1/.vimrc#L495-L497
nmap <expr> _ getline('.')[0 : col('.') - 2] =~# '^\s\+$' ? '0' : '^'
xmap <expr> _ getline('.')[0 : col('.') - 2] =~# '^\s\+$' ? '0' : '^'
